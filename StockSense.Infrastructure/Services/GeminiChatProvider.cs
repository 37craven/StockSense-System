...
public sealed class GeminiChatProvider : IAiChatProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var systemPrompt =
            "You are StockSense Assistant. Answer only using the StockSense records provided. " +
        ...
        try
        {
            ...
            using var response = await HttpClient.PostAsJsonAsync(endpoint, requestBody, cancellationToken);
            if (!response.IsSuccessStatusCode)
            {
                SafeLogWarning("Gemini request failed with {StatusCode}. Check the configured Gemini API key, project status, and model.", response.StatusCode);
                return null;
            }
        ...
        {
            SafeLogWarning(exception, "Gemini grounded answer generation failed. Falling back to local StockSense response.");
            return null;
        }
    }

    public async Task<string?> GenerateGeneralMotorcycleAnswerAsync(
        string userQuestion,
        string intent,
        CancellationToken cancellationToken = default)
    {
        ...
        var requestBody = CreateGenerateContentRequest(systemPrompt, userPrompt, temperature: 0.35, maxOutputTokens: 650);
        return await SendGenerateContentRequest(requestBody, "Gemini general motorcycle answer generation failed. Falling back to local StockSense response.", cancellationToken);
    }
    ...
    private static object CreateGenerateContentRequest(string systemPrompt, string userPrompt, double temperature, int maxOutputTokens) => new
    {
        system_instruction = new
        {
            parts = new[] { new { text = systemPrompt } }
    ...
    private async Task<string?> SendGenerateContentRequest(object requestBody, string failureMessage, CancellationToken cancellationToken)
    {
        try
        {
            var endpoint = $"https://generativelanguage.googleapis.com/v1beta/models/{Uri.EscapeDataString(_model)}:generateContent?key={Uri.EscapeDataString(_apiKey!)}";
